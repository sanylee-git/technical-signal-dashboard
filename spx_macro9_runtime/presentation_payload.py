"""Presentation-only payload for the independent S&P Macro9 runtime."""
...
from .frozen_replay import EVALUATION_START
from .live_replay import _metrics, performance_calendar, replay_combo1_raw_history, replay_core_chart_history
...
def _final10(final: pd.DataFrame) -> pd.DataFrame:
    out = final.copy().sort_values("display_order", kind="mergesort").reset_index(drop=True)
    out["model_family"] = out["family"].astype(str).str.upper()
    ...
    for parent in final.itertuples(index=False):
        ...
        if parent.model_family == "COMBO1":
            for order, component_id in enumerate(str(parent.component_ids_key).split("|"), start=1):
                ...
                if component_id not in registry.index:
                    continue
                ...
                detail.insert(4, "component_label", f"{meta['indicator_family']} · {component_id.split('__')[-1]}")
                parts.append(detail)
        else:
...
def _benchmark_history(panel: pd.DataFrame, final: pd.DataFrame, basis_date: object) -> pd.DataFrame:
    base = _normalized(panel[["date", "close"]])
    base = base.loc[base["date"].le(pd.Timestamp(basis_date).normalize())]
    parts = []
...
def _display_metrics(runtime: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, str]]:
    ...
    for window, start in windows.items():
        ...
        hold_rows.append({"candidate_id": "SPX_HOLD", "window": window, **_metrics(np.zeros(n, dtype=bool), returns[start:evaluation_end + 1], dates[start:evaluation_end + 1], 1.0)})
    official = runtime["metrics"].copy()
    if "completed_cycle_count" not in official.columns and "risk_off_cycle_count" in official.columns:
        official["completed_cycle_count"] = official["risk_off_cycle_count"]
    official["window"] = "FULL"
    official["asset"] = 100.0 * (1.0 + pd.to_numeric(official["total_return"], errors="coerce"))
    history = runtime["history"].copy()
    history["date"] = pd.to_datetime(history["date"]).dt.normalize()
    history = history.loc[history["date"].le(cutoff)]
    ten_rows: list[dict[str, object]] = []
    for candidate_id in official["candidate_id"].astype(str):
        states = history.loc[history["candidate_id"].eq(candidate_id)].sort_values("date")
        states = states.set_index("date").reindex(dates[ten_start : evaluation_end + 1])
        state = states["strategy_risk_state"].to_numpy(dtype=float)
        state = np.nan_to_num(state, nan=1.0).astype(bool)
        prior_row = history.loc[history["candidate_id"].eq(candidate_id)].sort_values("date")
        prior = 0.0 if not prior_row.empty and bool(prior_row.iloc[0]["previous_strategy_risk_state"]) else 1.0
        values = _metrics(state, returns[ten_start : evaluation_end + 1], dates[ten_start : evaluation_end + 1], prior)
        ten_rows.append({"candidate_id": candidate_id, "window": "10Y", **values, "asset": 100.0 * (1.0 + values["total_return"])})
    metrics = pd.DataFrame(ten_rows + official.to_dict("records"))
    hold = pd.DataFrame(hold_rows)
    hold["asset"] = 100.0 * (1.0 + hold["total_return"])
    return metrics, hold, {
...
def build_presentation_payload(runtime: dict[str, Any]) -> dict[str, Any]:
    """Build S&P display data from one S&P-owned Frozen-prefix/live-tail run."""
    if runtime.get("runtime_mode") not in {"FROZEN_ONLY", "FROZEN_PREFIX_LIVE_TAIL"}:
        raise RuntimeError("S&P Macro9 presentation runtime contract failed")
    final = _final10(runtime["final10"])
    ...
    snapshot["n_or_m"] = final["n_or_m"].to_numpy()
    ...
    confirmed_snapshot["n_or_m"] = final["n_or_m"].to_numpy()
