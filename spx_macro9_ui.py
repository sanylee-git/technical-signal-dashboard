"""S&P Macro9 presentation renderer backed by its independent Live runtime."""
...
def _main_chart(payload: dict[str, Any], candidate_id: str, basis: object, years: int | str) -> go.Figure | None:
    ...
    fig.add_trace(go.Scatter(x=benchmark["date"], y=benchmark["close"], name="S&P 500", line=dict(color="#BDBDBD", width=1.6)))
    for column, name, color, symbol in (("risk_start", "Risk 시작", "#F05A47", "triangle-down"), ("risk_end", "Risk 종료", "#60A5FA", "triangle-up")):
        ...
        if not events.empty:
            ...
            fig.add_trace(go.Scatter(x=points["date"], y=points["close"], name=name, mode="markers", marker=dict(color=color, size=10, symbol=symbol)))
    row = payload["final10"].loc[payload["final10"]["candidate_id"].eq(candidate_id)].iloc[0]
...
def _component_chart(payload: dict[str, Any], parent_id: str, component_id: str, component_kind: str, basis: object, years: int | str, *, show_aux: bool) -> go.Figure | None:
    ...
    if component_kind == "CHILD_COMBO1_RAW_STATE":
        fig.add_trace(go.Scatter(x=benchmark["date"], y=benchmark["close"], name="S&P 500", line=dict(color="#BDBDBD", width=1.5)))
    else:
        ...
        fig.add_trace(go.Scatter(x=benchmark["date"], y=benchmark["close"], name="S&P 500", yaxis="y2", line=dict(color="#777777", width=1)))
...
def _backtest_table(payload: dict[str, Any], family: str, selected_id: str, final: pd.DataFrame | None = None) -> str:
    ...
    rows = ["<tr>" + "".join([
        "<td style='padding:7px 8px;color:#EDEDED;font-weight:700;text-align:left;white-space:nowrap'>S&P 500 홀드</td>",
    ...
    with container:
        ...
        if payload is None:
            try:
                ...
                st.error(f"S&P Macro9 Live 데이터를 준비하지 못했습니다: {exc}")
                return
        if not isinstance(payload, dict) or payload.get("ui_side_model_calculation_count") != 0:
            st.error("S&P Macro9 presentation contract 검증 실패")
            return
        ...
        with c2:
            st.selectbox("기준지수", ["S&P 500"], key="macro9_spx_benchmark", disabled=True, label_visibility="collapsed")
        with c3:
            period = st.select_slider("기간", options=options, value=5 if 5 in options else options[0], key="macro9_spx_years", label_visibility="collapsed", format_func=lambda value: "전체" if value == "all" else f"{value}년")
        with c4:
            st.markdown('<div class="macro2-control-spacer"></div>', unsafe_allow_html=True)
            show_aux = st.checkbox("보조선 표시", value=False, key="macro9_spx_show_raw", label_visibility="collapsed")
        ...
        with controls:
            st.multiselect("조합 지표", components, default=components, disabled=True, key="macro9_spx_selected_components", label_visibility="collapsed", format_func=lambda component_id: str(component_history.loc[component_history["component_id"].eq(component_id)].iloc[0]["component_label"]))
        with criteria:
        ...
        if main is None:
            ...
            st.plotly_chart(main, width="stretch", config={"displayModeBar": False}, key=f"macro9_spx_main_chart_{candidate_id}_{period}")
        st.markdown('<div class="macro2-divider"></div>', unsafe_allow_html=True)
        for index, component_id in enumerate(components):
            ...
            with st.expander(f"{index + 1}. {row.component_label}", expanded=True):
                ...
                if chart is None:
                    ...
                    st.plotly_chart(chart, width="stretch", config={"displayModeBar": False}, key=f"macro9_spx_component_chart_{candidate_id}_{index}_{period}")
