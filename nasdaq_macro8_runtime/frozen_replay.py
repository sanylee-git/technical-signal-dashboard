"""Deterministic NASDAQ Core15/Final20 replay from dashboard-owned Frozen assets."""
...
def _dynamic_signal(series: pd.Series, params: dict[str, object], event_allowed: pd.Series | None = None) -> pd.DataFrame:
    ...
    out = out.dropna().copy()
    minimum = max(20, int(params["window"]) // 2)
    out["start_line"] = out["ema"].rolling(int(params["window"]), min_periods=minimum).quantile(float(params["start_q"])).shift(1)
    out["end_line"] = out["ema"].rolling(int(params["window"]), min_periods=minimum).quantile(float(params["end_q"])).shift(1)
    out = out.dropna().copy()
    if out.empty:
        return out
    previous_ema = out["ema"].shift(1)
    start = (previous_ema >= out["start_line"].shift(1)) & (out["ema"] < out["start_line"])
    end = (previous_ema <= out["end_line"].shift(1)) & (out["ema"] > out["end_line"])
...
def _core_detail_signal(panel: pd.DataFrame, record: pd.Series) -> pd.DataFrame:
    ...
    family = str(record.indicator_family)
    if family == "us_10y_slope":
        signal = _dynamic_signal(-_rolling_slope(_panel_series(panel, "dgs10_available"), int(params["slope_window"])), params)
    elif family == "ndx_rsi":
        signal = _rsi_signal(_panel_series(panel, "ndx_close"), params)
    else:
        close = _panel_series(panel, "ndx_close")
        high = _panel_series(panel, "ndx_high")
        low = _panel_series(panel, "ndx_low")
        ...
        if family == "ndx_bollinger":
            signal = _bollinger_signal(close.where(ohlc), high.where(ohlc), low.where(ohlc), params, event_allowed)
        elif family == "ndx_natr":
            source = -(100.0 * _wilder_atr(high.where(ohlc), low.where(ohlc), close, int(params["natr_period"])) / close)
            signal = _dynamic_signal(source, params, event_allowed)
        elif family == "ndx_hv":
            valid_close = close.where(close > 0.0)
            source = -(np.log(valid_close / valid_close.shift(1)).rolling(int(params["hv_period"]), min_periods=int(params["hv_period"])).std(ddof=1) * np.sqrt(252.0))
...
def replay_core_chart_history(panel: pd.DataFrame, registry: pd.DataFrame) -> pd.DataFrame:
    ...
    for record in registry.sort_values("candidate_id", kind="mergesort").itertuples(index=False):
        ...
        chart.insert(1, "component_label", f"{record.indicator_name} · {str(record.candidate_id).split('__')[-2]}")
...
def _replay_final20(panel: pd.DataFrame, final20: pd.DataFrame, children: pd.DataFrame, core: dict[str, dict[str, np.ndarray]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for record in final20.sort_values("display_order", kind="mergesort").itertuples(index=False):
        ...
        if record.family == "Combo1":
            ...
            for child_record in child.itertuples(index=False):
                ...
                child_raw[child_id], child_starts[child_id], _child_active = _build_combo1_raw(ids, int(child_record.child_K), int(child_record.child_L), raw_performance, valid_performance)
            history_start = max(child_starts.values())
            ordered_child_ids = list(child_raw)
            active = np.vstack([child_raw[item][history_start : eval_end + 1] for item in ordered_child_ids]).sum(axis=0, dtype=np.int16)
            combo_raw = _hysteresis(active, int(record.K), int(record.L)).astype(np.int8)
            final_state = np.full(len(dates), -1, dtype=np.int8)
            active_full = np.full(len(dates), -1, dtype=np.int16)
            final_state[history_start + 1 : eval_end + 1] = combo_raw[:-1]
            active_full[history_start : eval_end + 1] = active
            history_start += 1
            state = final_state
            evaluation_state = state[eval_start : eval_end + 1]
        ...
        if (evaluation_state < 0).any() or state[eval_start - 1] < 0:
            raise RuntimeError(f"Final20 state unresolved: {candidate_id}")
